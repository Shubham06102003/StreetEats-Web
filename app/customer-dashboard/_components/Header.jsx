...
import { collection, query, where, getDocs, updateDoc,doc, setDoc } from "firebase/firestore"; // Import necessary Firestore functions
...
function Header() {
      ...
      navigator.geolocation.getCurrentPosition(
        async (position) => {
          ...
          try {
            ...
            setLocation(fullLocation);
            setSearchQuery(fullLocation);
            handleLocationChange(fullLocation); // Add location to Firestore
          } catch (error) {
            console.error("Error detecting location:", error);
            setLocation("Location Unavailable");
          } finally {
  ...
  const handleLocationChange = async (newLocation) => {
    ...
    if (user) {
      ...
      setUserEmail(cus_email);

        try {
          // Reference to the customer's document based on the user's email
          const customerRef = collection(db, "customers");
          const q = query(customerRef, where("cus_email", "==", cus_email));
          ...
          if (!querySnapshot.empty) {
            ...
            console.log("Customer location updated in Firestore");
          } else {
            // If the user doesn't exist, create a new document with cus_email as the document ID
            const docRef = doc(db, "customers", cus_email); // Use cus_email as the document ID
  ...
  const handlePlaceChanged = () => {
    if (autocompleteRef.current) {
      ...
      setSearchQuery(fullAddress);
      setShowDropdown(false);
      handleLocationChange(fullAddress); // Add location to Firestore
    }
  };
